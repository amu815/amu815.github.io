...
const kindTone: Record<EducationKind, { dot: string; ring: string; text: string }> = {
  doctoral: { dot: "bg-orange", ring: "ring-orange/30", text: "text-orange" },
    ...
    <div className="flex flex-col gap-6">
      ...
      <ol className="relative ml-2 border-l border-border-strong pl-6">
        {education.map((e, i) => {
          ...
          const range = e.planned
            ? `${formatYearMonth(e.enrolled, lang)} – `
            : e.graduated
          ...
          return (
            <li
              key={e.id}
              className={`relative pb-6 fade-in-up ${e.planned ? "opacity-90" : ""}`}
              style={{ animationDelay: `${i * 70}ms` }}
            >
              <span
                className={`absolute -left-[33px] top-1.5 inline-flex h-3 w-3 rounded-full ring-4 ${tone.dot} ${tone.ring} ${
                  e.planned ? "opacity-70" : ""
                }`}
                aria-hidden
              />
              <div
                className={`surface-card flex flex-col gap-1 p-4 sm:p-5 ${
                  e.planned ? "border-dashed" : ""
                }`}
              >
